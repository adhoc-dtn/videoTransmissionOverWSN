...
#        使い方 : $ perl recursive_videoSender.pl (実行プログラム) (interface) (port) (初期ビットレート) (パターン数）　(１パターンあたりの試行回数)
...
while ($try < $loop_max) {
    ...
    printf("(videoSender) EXPERIMENT (bitrate ${bitrate}  try ${pattern} port ${port} starts now (alltry ${try}/${loop_max}");
